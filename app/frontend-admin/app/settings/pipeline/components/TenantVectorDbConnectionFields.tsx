  ...
  return (
    <div>
      ...
      <TextInput
        value={value}
        onChange={onChange}
        placeholder={placeholder ?? "e.g. QDRANT_API_KEY"}
      />
      <p className="text-[11px] text-slate-500 mt-1">
        Name of the environment variable holding the secret — not the key value itself.
      </p>
    </div>
  );
}
      ...
      case "weaviate":
        return (
          <>
            ...
            <EnvKeyField
              label="API key env var (WCS / auth)"
  ...
  return (
    <div className="mt-4 rounded-xl border border-violet-200 bg-violet-50/30 p-4 space-y-4">
      ...
      <p className="text-[11px] text-slate-500 border-t border-violet-100 pt-2">
        API keys are referenced by env var name in Client JSON; set the actual secret in{" "}
        <Link href="/settings" className="text-primary-700 underline-offset-2 hover:underline">
