...
# ─────────────────────────────────────────────────────────────────────────────
# MASTER GUARD — skip all broker interaction when CELERY_ENABLED != true.
# This prevents Celery from attempting to connect to a broker at import time
# when the user has explicitly disabled distributed task processing.
# ─────────────────────────────────────────────────────────────────────────────
...
if is_celery_enabled():
    ...
    logger.info("[celery_app] Celery application initialised (broker=%s)", _BROKER_URL.split("@")[-1] if "@" in _BROKER_URL else _BROKER_URL)

else:
    # ── CELERY DISABLED — no broker connection, no Celery import ─────────
    celery_app = None  # type: ignore[assignment]
    logger.info("[celery_app] Celery is DISABLED (CELERY_ENABLED!=true) — skipping broker connection")
