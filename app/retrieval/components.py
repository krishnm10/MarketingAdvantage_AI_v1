...
def resolve_runtime_components(config: ClientConfig) -> RuntimeComponents:
    ...
    return RuntimeComponents(
        ...
        reranker_name=_reranker.type.value if _reranker else "none",
        reranker_model=_reranker.model if _reranker else "",
...
def resolve_runtime_components_legacy() -> RuntimeComponents:
    """
    Build RuntimeComponents from environment variables ONLY.
    Used exclusively under ENABLE_LEGACY_ENV_FALLBACK for backward compat.
    """
    return RuntimeComponents(
        ...
        reranker_name=os.getenv("MAI_RERANKER", "none").strip().lower(),
        reranker_model=os.getenv("MAI_RERANKER_MODEL", ""),
        search_mode="semantic",
        enable_hyde=False,
        enable_reranking=os.getenv("MAI_RERANKER", "none").strip().lower() not in ("none", "", "disabled"),
        hybrid_alpha=0.7,
...
def instantiate_llm(
    ...
    if provider == "ollama":
        ...
        return OllamaLLM(model=model), model
    ...
    if provider in ("gemini", "google"):
        key = os.getenv(api_key_env or "GEMINI_API_KEY", "")
        if not key:
            raise ValueError(
                f"API key env var '{api_key_env or 'GEMINI_API_KEY'}' is not set "
                "for Gemini LLM."
            )
